...
def make_supply_schedule(subsession,constants):
    '''Build the supply schedule. Its  length is equal to the maximum number of bids that can 
       possibly be entered.'''
    this_session_config = subsession.session.config
    num_low_emitters = this_session_config['num_low_emitters']
    num_high_emitters = this_session_config['num_high_emitters']
    initial_ecr_reserve_amount = this_session_config['initial_ecr_reserve_amount']
    ecr_reserve_amount = this_session_config['initial_ecr_reserve_amount']
    ecr_increment = this_session_config['ecr_reserve_increment']
    ecr_trigger_price = this_session_config['ecr_trigger_price']
    pcr_reserve_amount = this_session_config['price_containment_reserve_amount'] 
    ...
    pcr_trigger_price = this_session_config['price_containment_trigger']
    q_star = max(0,permits_available - ecr_reserve_amount)
    if this_session_config['ecr_treatment'] == 'Step':
        # Supply reduction in one big step
        supply_step = np.ones(initial_ecr_reserve_amount)*ecr_trigger_price
    elif this_session_config['ecr_treatment'] == 'Linear':
        ...
        #assert False
    elif this_session_config['ecr_treatment'] == 'Holt linear':
        # Smooth linear supply reduction
        # With reserve 5, ecr amount 18, ecr trigger 11
        #    things work out to $0.33 per allowance in the reserve
        first_increment = float((ecr_trigger_price-reserve_price)/ecr_reserve_amount)
        supply_step = np.linspace(float(reserve_price) + first_increment, float(ecr_trigger_price), num=ecr_reserve_amount, 
                                  endpoint=True,retstep=True)
        step_size = supply_step[1]
        supply_step = supply_step[0]
        # assert False
    elif this_session_config['ecr_treatment'] == 'None':
        supply_step = ''
...
def calculate_auction_price(these_bids,supply_curve,subsession,reserve_price):
    ...
    #this_session = subsession.session
    this_session_config = subsession.session.config
    bids = these_bids.sort_values(['bid'],ascending=False)
    num_bids = len(bids)
    ...
    if last_positive_bid_index < q_star - 1 or price == reserve_price:
